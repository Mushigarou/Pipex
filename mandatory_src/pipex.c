...
/*   Updated: 2023/03/07 23:08:39 by mfouadi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*look_for_path(char **av_cmd, char **env_path)
{
	...
	free(tmp);
	free_st(env_path, -1);
	msg("pipex: command not found:join_path\n", 127, 1);
	return (NULL);
}
...
char	*join_path(char **env_path, char *cmd)
{
	...
	i = 0;
	if (!cmd)
		msg("command not found:join_path\n", 1, 1);
	av_cmd = ft_split(cmd, ' ');
	if (!env_path)
	{
		free_st(env_path, -1);
